...
; 專案根目錄 (支援 src/ 開發與根目錄執行兩種情境)
global scriptRoot := (SubStr(A_ScriptDir, -3) = "src") ? A_ScriptDir . "\.." : A_ScriptDir

; INI檔案路徑
global iniFile := scriptRoot . "\Brown_Dust2_Settings.ini"
...
global langDir := scriptRoot . "\lang"  ; 語言包目錄
...
StartMainLoop() {
    ...
    while (loopRunning) {
        try {
            ...
            try {
                FileAppend(errorMsg . "`n", scriptRoot . "\error.log")
            }
            Sleep(100)
        }
